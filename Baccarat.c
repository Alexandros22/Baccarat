...
void main()
{
	...
	}while (x<0 || x>500);	// check of starting money //
	y=0;
	for (i=1;i<=4;i++)
	{
		for (j=1;j<=13;j++)
	...
	do
	{
		...
		do
		{
			...
			printf("\nCards still here:%d\n",i);
			if (x==0)			// if the money are over then the game ends //
				break;
			do
			{
		...
		}while (i>=6 && ans2=='C' && x!=0);	// repeat the game when we have cards, money, and do not choose stirring of the cards // 
        if (x==0)
		{
